...
A production-grade network management daemon built in Rust with a Matrix-style TypeScript/React dashboard. NetCtl provides transactional state management, idempotent VLAN/DHCP configuration, eBPF/XDP kernel-level QoS, and live metrics streaming.
...
### VLANs
```
POST   /api/vlan                   # Create new VLAN
GET    /api/vlan                   # List all VLANs
DELETE /api/vlan/:vlan_id          # Delete VLAN
```
...
### QoS
```
POST   /api/qos/device/:mac        # Set rate limit
DELETE /api/qos/device/:mac        # Remove rate limit
...
GET    /api/metrics/stream         # SSE live stream
```
...
### Backend
- **Database**: `/tmp/netctl.db` (configure in `main.rs`)
- **API Port**: `3001` (configure in `main.rs`)
- **Logging**: Set `RUST_LOG` environment variable
  ```bash
  RUST_LOG=debug cargo run
  ```

### Frontend
- **Backend URL**: See `frontend/vite.config.ts`
...
- Extend with JWT authentication (see future enhancements)
...
- **Per-App Traffic Shaping** - Application-level control
...
1. Code compiles without warnings (`cargo check`)
...
2. Review error messages and logs (`RUST_LOG=debug`)
3. Test in isolated environment first
4. Never run on production networks without testing
...
└──────────────────┬──────────────────────────┘
      ...
      │            │             │
┌─────▼───────┐ ┌─▼──────────┐ ┌▼────────────┐
│SQLite DB    │ │Transactional│ │eBPF/XDP    │
│State Store  │ │State Engine │ │Kernel QoS  │
└─────────────┘ └─────────────┘ └────────────┘
      │            │             │
      └────────────┼─────────────┘
                   │
...
- Core transactional state management
- VLAN and DHCP support
- Basic QoS framework
- REST API implementation
- Matrix-style React dashboard
- eBPF/XDP skeleton
